...
if __name__ == "__main__":
    ...
    country_names = get_country_names("apps/searchHotspots/search-logs")
    grid_sizes = [1, 0.5, 0.1]
    ...
    centers_df = pd.read_csv("apps/searchHotspots/data/countries_centers(clean).csv")

    ## MAIN LOOP, GENERATE INFO TO DRAW DATA FOR EACH COUNTRY
    for country in tqdm(country_names):
        print(f"Processing country: {country}....")

        try:
            coordinates_df = pd.read_csv(
                f"apps/searchHotspots/search-logs/search_logs_{country}.csv"
            )
            ...
            print(f"Error getting coordinates and center point of {country}")
            continue

        for grid_size in grid_sizes:
            try:
                country_ISO3 = pycountry.countries.get(alpha_2=country.lower()).alpha_3
                lat_range, lon_range = getBoundingBox(country_ISO3, countries_geos)
                grid = create_grid(lat_range, lon_range, grid_size)

                grid_sums = get_grid_sums(coordinates_df)

                cell_colors_sums = color_grid_sums(grid, grid_sums, color_map)

                save_data(grid_size, cell_colors_sums, country_ISO3, center)
            except:
